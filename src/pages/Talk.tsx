...
const initializeMemoryStore = (): MemoryStore => {
  ...
  if (!stored) {
    return { ...SEED_MEMORY_STORE };
  }
  try {
    const parsed = JSON.parse(stored) as Partial<MemoryStore>;
    const mergedEntities = [...SEED_MEMORY_STORE.entities];
...
const Talk: React.FC = () => {
  ...
  const [memorySidebarTab, setMemorySidebarTab] = useState<'list' | 'graph'>('list');
    ...
    switch (name) {
      ...
      case 'tag_memory': {
        ...
        if (entities.length === 0) {
          ...
          content = JSON.stringify({ success: true, message: `Tagged ${entity.name} as ${emotion}` });
          addToLog(`🏷️ Tagged: ${entity.name} as ${emotion}`);
        ...
        <div className="exit-overlay" onClick={() => setShowExitOverlay(false)}>
          <div className="exit-overlay-content" onClick={(e) => e.stopPropagation()}>
            <div className="exit-overlay-buttons">
              <button className="exit-btn save-btn" onClick={() => navigate('/remember')}>
                Save & Next
              </button>
